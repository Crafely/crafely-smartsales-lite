...
class DashboardApiHandler
{
    ...
    public function register_routes()
    {
        $routes = [
            ...
            '/dashboard/customers' => [
                'methods' => 'GET',
                'callback' => [$this, 'get_customer_analytics'],
            ],
            '/dashboard/products' => [
                'methods' => 'GET',
            ...
            '/dashboard/outlets' => [
                'methods' => 'GET',
                'callback' => [$this, 'get_outlet_analytics'],
            ],
            '/dashboard/recent-activities' => [
                'methods' => 'GET',
                'callback' => [$this, 'get_recent_activities'],
            ],
        ];

        foreach ($routes as $route => $config) {
            register_rest_route('ai-smart-sales/v1', $route, [
    ...
    private function get_orders_by_outlet($orders, $outlet_id)
    {
        ...
        return array_filter($orders, function ($order) use ($outlet_id) {
    ...
    public function get_dashboard_summary($request)
    {
        try {
            ...
            foreach ($orders as $order) {
                ...
                if (taxonomy_exists('generic_channel')) {
                    $channels = wp_get_post_terms($order->get_id(), 'generic_channel');
            ...
            // Get customer stats
            $total_customers = count(get_users(['role' => 'customer']));
    ...
    public function get_sales_analytics($request)
    {
        ...
        foreach ($orders as $order) {
            ...
            foreach ($order->get_items() as $item) {
                ...
                if (!isset($best_selling_products[$product_id])) {
                    $best_selling_products[$product_id] = [
                        'id' => $product_id,
                        'name' => $item->get_name(),
                        'quantity' => 0,
                        'total' => 0,
        ...
        uasort($best_selling_products, function ($a, $b) {
    ...
    public function get_outlet_analytics($request)
    {
        ...
        uasort($outlet_stats, function ($a, $b) {
