...
class InvoiceApiHandler
{
    ...
    public function register_routes()
    {
        ...
        register_rest_route('ai-smart-sales/v1', '/invoices', [
            [
                'methods'  => 'GET',
        ...
        register_rest_route('ai-smart-sales/v1', '/invoices/(?P<id>\d+)', [
            [
                'methods'  => 'GET',
        ...
        // Trash operations
        register_rest_route('ai-smart-sales/v1', '/invoices/trash', [
            'methods'  => 'GET',
            'callback' => [$this, 'get_trash_invoices'],
            'permission_callback' => [$this, 'check_permission'],
        ]);

        // Bulk operations
        register_rest_route('ai-smart-sales/v1', '/invoices/restore', [
            'methods'  => 'PUT',
            'callback' => [$this, 'bulk_restore_invoices'],
            'permission_callback' => [$this, 'check_permission'],
        ]);

        register_rest_route('ai-smart-sales/v1', '/invoices/bulk-delete', [
            'methods'  => 'DELETE',
    ...
    private function format_invoice_response($invoice)
    {
        ...
        $processed_line_items = array_map(function ($item) {
            ...
            return $line_item;
        }, is_array($line_items) ? $line_items : []);
    ...
    public function create_invoice($request)
    {
        ...
        update_post_meta($invoice_id, 'line_items', json_encode($data['line_items']));
    ...
    public function get_invoices($request)
    {
        ...
        $invoices = array_map([$this, 'format_invoice_response'], $query->posts);
    ...
    public function get_invoice($request)
    {
        ...
        if (!$invoice || $invoice->post_type !== 'smartsales_invoice') {
            return new WP_REST_Response($this->format_error_response(
                'Invoice not found.',
                ['id' => "Invoice with ID {$invoice_id} does not exist."],
                404
            ), 404);
        }
    ...
    public function delete_invoice($request)
    {
        ...
        if (!$result) {
            return new WP_REST_Response($this->format_error_response(
                ...
                ['id' => "Could not delete invoice with ID {$invoice_id}."],
                500
            ), 500);
        }
    ...
    public function bulk_restore_invoices($request)
    {
        ...
        if (empty($data['ids']) || !is_array($data['ids'])) {
            return new WP_REST_Response($this->format_error_response(
                'Invalid request.',
                ['ids' => 'Invoice IDs array is required.'],
                400
            ), 400);
        }
