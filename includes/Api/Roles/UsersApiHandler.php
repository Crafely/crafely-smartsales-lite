 ...
 *
 * Handles REST API requests related to users and their roles in the Crafely Smart Sales Lite plugin.
 */
class UsersApiHandler {

	/**
	 * Valid roles for POS users.
	 *
	 * @var array
	 */
	...
	/**
	 * API namespace for the Crafely Smart Sales Lite plugin.
	 *
	 * @var string
	 */
	...
	public function register_routes() {
		...
		foreach ( $routes as $route => $endpoints ) {
			foreach ( $endpoints as [$method, $callback] ) {
				...
				$permission_callback = ( '/users/logout' === $route )
	...
	public function check_permission( $request ) {
		...
		if ( in_array($request->get_method(), [ 'POST', 'PUT', 'DELETE' ], true) ) {
	...
	private function format_user_response( $user ) {
		...
		if ( empty($status) ) {
			$status = 0 === $user->user_status ? 'active' : 'inactive';
		}
		...
		if ( $outlet_id ) {
			...
			if ( $outlet ) {
				...
				$response['outlet'] = [
					...
					'status'       => get_post_meta($outlet_id, 'outlet_status', true) ? get_post_meta($outlet_id, 'outlet_status', true) : 'active',
			...
			if ( $counter_id ) {
				...
				if ( $counter ) {
					...
					$response['counter'] = [
						'id'          => $counter_id,
						'name'        => $counter->post_title,
						'status'      => get_post_meta($counter_id, 'counter_status', true) ? get_post_meta($counter_id, 'counter_status', true) : 'active',
		...
		$response['stats'] = [
			...
			'last_activity'  => get_user_meta($user->ID, 'last_activity', true) ? get_user_meta($user->ID, 'last_activity', true) : gmdate('Y-m-d H:i:s'),
	...
	private function get_user_permissions( $user ) {
		...
		} elseif ( in_array('csmsl_pos_outlet_manager', $user->roles, true ) ) {
			$permissions = [
				'manage_users'     => false,
				'manage_outlets'   => true,
				'manage_inventory' => true,
	...
	private function get_counter_stats( $counter_id ) {
		...
		return [
			'today_sales'    => floatval($today_sales) ? floatval($today_sales) : 0,
			'month_sales'    => floatval($month_sales) ? floatval($month_sales) : 0,
			'total_orders'   => intval($total_orders) ? intval($total_orders) : 0,
			'last_active'    => get_post_meta($counter_id, 'last_active', true),
			'total_cashiers' => count(get_post_meta($counter_id, 'counter_assignment_history', true) ? get_post_meta($counter_id, 'counter_assignment_history', true) : []),
		];
	}
