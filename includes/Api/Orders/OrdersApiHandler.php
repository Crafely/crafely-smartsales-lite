...
class OrdersApiHandler
{
    ...
    public function register_routes()
    {
        ...
        register_rest_route('ai-smart-sales/v1', '/orders', [
            [
                'methods'  => 'GET',
        ...
        register_rest_route('ai-smart-sales/v1', '/orders/(?P<id>\d+)', [
            [
                'methods'  => 'GET',
        ...
        // Bulk operations
        register_rest_route('ai-smart-sales/v1', '/orders/restore', [
            'methods'  => 'PUT',
            'callback' => [$this, 'bulk_restore_orders'],
            'permission_callback' => [$this, 'check_permission'],
        ]);

        register_rest_route('ai-smart-sales/v1', '/orders/bulk-delete', [
            'methods'  => 'DELETE',
            'callback' => [$this, 'bulk_delete_orders'],
            'permission_callback' => [$this, 'check_permission'],
        ]);

        // Trash operations
        register_rest_route('ai-smart-sales/v1', '/orders/trash', [
            'methods'  => 'GET',
            'callback' => [$this, 'get_trash_orders'],
            'permission_callback' => [$this, 'check_permission'],
        ]);
    }
    ...
    public function check_permission($request)
    {
        ...
        if (
            in_array($request->get_method(), ['DELETE', 'PUT']) &&
            !array_intersect(['administrator', 'aipos_outlet_manager'], $user_roles)
        ) {
            return false;
        }
    ...
    private function format_order_response($order)
    {
        ...
        $channels = wp_get_post_terms($order->get_id(), 'generic_channel', ['fields' => 'slugs']);
        ...
        if ($is_website_order) {
            ...
            if ($creator) {
                ...
                else if (!empty($creator->display_name)) {
        ...
        $payment_details = $split_payments ?
            ['payment_method' => null, 'split_payments' => json_decode($split_payments, true)] :
            ['payment_method' => $order->get_payment_method(), 'split_payments' => null];
    ...
    public function get_orders($request)
    {
        ...
        $is_cashier = in_array('aipos_cashier', $user_roles) &&
            !array_intersect(['administrator', 'aipos_outlet_manager', 'aipos_shop_manager'], $user_roles);
        ...
        $data = array_map([$this, 'format_order_response'], $orders);
    ...
    public function create_order($request)
    {
        ...
        if (!isset($data['split_payments'])) {
            ...
            $valid_payment_methods = ['cash', 'card', 'bank_transfer', 'paypal', 'upi', 'cryptocurrency', 'cod'];
        ...
        if (isset($data['split_payments'])) {
            if (!is_array($data['split_payments'])) {
                ...
                foreach ($data['split_payments'] as $index => $payment) {
                    ...
                    if (!in_array($method, ['cash', 'card', 'bank_transfer', 'paypal', 'upi', 'cryptocurrency', 'cod'])) {
        ...
        if (isset($data['split_payments'])) {
            $order->update_meta_data('_split_payments', json_encode($data['split_payments']));
        ...
        if (in_array('aipos_outlet_manager', $current_user->roles)) {
            $pos_channel = get_term_by('slug', 'pos-system', 'generic_channel');
            if ($pos_channel) {
                wp_set_object_terms($order->get_id(), $pos_channel->term_id, 'generic_channel');
            }
        }
