...
class AuthManager
{
    private const POS_ROLES = ['aipos_cashier', 'aipos_outlet_manager', 'aipos_shop_manager'];

    public function __construct()
    {
        add_filter('woocommerce_prevent_admin_access', [$this, 'allow_pos_admin_access'], 20);
        add_filter('woocommerce_login_redirect', [$this, 'handle_login_redirect'], 20, 2);
        add_filter('login_redirect', [$this, 'handle_wp_login_redirect'], 20, 3);
        add_action('init', [$this, 'handle_login']);
        add_action('admin_init', [$this, 'enforce_access_restrictions'], 5);
    }
    ...
    public function should_redirect_to_pos($user)
    {
        ...
        $admin_roles = ['administrator', 'editor', 'author', 'shop_manager'];
    ...
    {
        if (!$user || !isset($user->roles)) return $redirect;
    ...
    {
        set_transient('pos_login_error', $message, 30);
