...
class Admin {
	...
	public function __construct() {
		...
		add_action( 'admin_menu', array( $this, 'smartsales_add_admin_menu' ) );

		// Enqueue admin scripts and styles
		add_action( 'admin_enqueue_scripts', array( $this, 'smartsales_enqueue_admin_assets' ) );

		// Add script loader tag filter for module type
		add_filter( 'script_loader_tag', array( $this, 'smartsales_add_module_type_to_script' ), 10, 3 );
	}
	...
	private function smartsales_enqueue_vue_assets() {
		...
		if ( ! empty( $js_files ) ) {
			...
			wp_enqueue_script( 'ai-smart-sales-main-js', $js_url, array(), filemtime( $js_files[0] ), true );
		...
		if ( ! empty( $css_files ) ) {
			...
			wp_enqueue_style( 'ai-smart-sales-css', $css_url, array(), filemtime( $css_files[0] ) );
		}
	...
	public function smartsales_add_admin_menu() {
		...
		add_menu_page(
			...
			'smartsales',
			array( $this, 'smartsales_render_dashboard_page' ),
	...
	public function smartsales_render_dashboard_page() {
		// The API settings are already localized in smartsales_localize_admin_scripts()
