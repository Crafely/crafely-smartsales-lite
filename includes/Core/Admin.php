<?php

namespace AISMARTSALES\Includes\Core;
if (!defined('ABSPATH')) {
    exit;
}

class Admin
{

    public function __construct()
    {
        if (!defined('SMARTSALES_DIR') || !defined('SMARTSALES_URL')) {
            wp_die(esc_html__('SMARTSALES_DIR or SMARTSALES_URL is not defined.', 'smartsales-lite'));
        }

        // Add admin menu and settings
        add_action('admin_menu', [$this, 'smartsales_add_admin_menu']);

        // Enqueue admin scripts and styles
        add_action('admin_enqueue_scripts', [$this, 'smartsales_enqueue_admin_assets']);

        // Add script loader tag filter for module type
        add_filter('script_loader_tag', [$this, 'smartsales_add_module_type_to_script'], 10, 3);
    }

    /**
     * Enqueue admin scripts and styles.
     */
    public function smartsales_enqueue_admin_assets($hook)
    {
        // Keep existing styles for app page
        if ($hook === 'toplevel_page_smartsales') {

            // Enqueue scoped reset CSS
            wp_enqueue_style(
                'ai-smart-sales-reset-css',
                plugin_dir_url(dirname(__FILE__, 2)) . 'assets/css/reset.css',
                [],
                filemtime(plugin_dir_path(dirname(__FILE__, 2)) . 'assets/css/reset.css')
            );
            // Enqueue Admin JS
            wp_enqueue_script(
                'ai-smart-sales-admin-js',
                plugin_dir_url(dirname(__FILE__, 2)) . 'assets/js/admin.js',
                ['jquery', 'wp-i18n', 'wp-components', 'wp-element', 'wp-api-fetch'],
                filemtime(plugin_dir_path(dirname(__FILE__, 2)) . 'assets/js/admin.js'),
                true
            );

            // Enqueue Vue app assets
            $this->smartsales_enqueue_vue_assets();
        }
    }


    private function smartsales_enqueue_vue_assets()
    {
        $js_files = glob(SMARTSALES_DIR . 'dist/js/main.*.js');
        $css_files = glob(SMARTSALES_DIR . 'dist/css/main.*.css');

        // Enqueue Vue app's main JS
        if (!empty($js_files)) {
            $js_url = SMARTSALES_URL . 'dist/js/' . basename($js_files[0]);
            wp_enqueue_script('ai-smart-sales-main-js', $js_url, [], filemtime($js_files[0]), true);

            // Localize script with WordPress API settings
            $this->smartsales_localize_admin_scripts();
        }

        // Enqueue Vue app's main CSS
        if (!empty($css_files)) {
            $css_url = SMARTSALES_URL . 'dist/css/' . basename($css_files[0]);
            wp_enqueue_style('ai-smart-sales-css', $css_url, [], filemtime($css_files[0]));
        }

        // Enqueue Quill Editor CSS
        wp_enqueue_style(
            'quill-editor-bubble-css',
            SMARTSALES_URL . 'assets/quill-editor/quill.bubble.css',
            [],
            filemtime(SMARTSALES_DIR . 'assets/quill-editor/quill.bubble.css')
        );

        wp_enqueue_style(
            'quill-editor-snow-css',
            SMARTSALES_URL . 'assets/quill-editor/quill.snow.css',
            [],
            filemtime(SMARTSALES_DIR . 'assets/quill-editor/quill.snow.css')
        );
    }


    /**
     * Add the admin menu for the plugin.
     */
    public function smartsales_add_admin_menu()
    {
        // Check if user has required role
        $user = wp_get_current_user();
        $allowed_roles = ['administrator', 'aipos_shop_manager', 'aipos_outlet_manager'];

        if (!array_intersect($allowed_roles, (array) $user->roles)) {
            return;
        }

        add_menu_page(
            esc_html__('Smart sales lite', 'smartsales-lite'),
            esc_html__('Smart Sales Lite', 'smartsales-lite'),
            'read', // Use 'read' instead of 'manage_options' to allow POS managers
            'smartsales',
            [$this, 'smartsales_render_dashboard_page'],
            'data:image/svg+xml;base64,',
            30
        );
    }

    /**
     * Render the Vue Dashboard page.
     */
    public function smartsales_render_dashboard_page()
    {
        // Ensure wpApiSettings is available
        $api_data = array(
            'root' => esc_url_raw(rest_url()),
            'nonce' => wp_create_nonce('wp_rest'),
            'baseUrl' => esc_url_raw(SMARTSALES_URL),
            'assetsUrl' => esc_url_raw(SMARTSALES_URL . 'dist/free/'),
            'isLoggedIn' => is_user_logged_in(),
            'currentUser' => is_user_logged_in() ? wp_get_current_user()->ID : 0,
            'userRoles' => is_user_logged_in() ? wp_get_current_user()->roles : [],
            'apiNamespace' => 'ai-smart-sales/v1',
        );

        // Output the API settings inline to ensure they're available
        echo '<script>window.wpApiSettings = ' . json_encode($api_data) . ';</script>';
        echo '<div id="app"></div>';
    }

    /**
     * Localize admin scripts with WordPress API settings
     */
    private function smartsales_localize_admin_scripts()
    {
        // Create the same API data structure as in the POS template
        $api_data = array(
            'root' => esc_url_raw(rest_url()),
            'nonce' => wp_create_nonce('wp_rest'),
            'baseUrl' => esc_url_raw(SMARTSALES_URL),
            'assetsUrl' => esc_url_raw(SMARTSALES_URL . 'dist/free/'),
            'isLoggedIn' => is_user_logged_in(),
            'currentUser' => is_user_logged_in() ? wp_get_current_user()->ID : 0,
            'userRoles' => is_user_logged_in() ? wp_get_current_user()->roles : [],
            'apiNamespace' => 'ai-smart-sales/v1',
        );

        // Make wpApiSettings available globally for the admin dashboard
        wp_add_inline_script(
            'ai-smart-sales-main-js',
            'window.wpApiSettings = ' . json_encode($api_data) . ';',
            'before'
        );
    }

    /**
     * Add module type to specific scripts using WordPress-compliant method
     */
    public function smartsales_add_module_type_to_script($tag, $handle, $src)
    {
        // Only modify our specific script
        if ($handle === 'ai-smart-sales-main-js') {
            // Replace the script tag to include type="module"
            $tag = str_replace('<script ', '<script type="module" ', $tag);
        }
        return $tag;
    }
}