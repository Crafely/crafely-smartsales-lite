<script lang="ts" setup>
    import AppLayout from '@/layout/AppLayoutWithResize.vue'
    import { useCustomer } from './useCustomer'
    import { ConfirmDialog, useConfirmDialog } from '@/components/confirmDialog'
    import CustomerDataTable from './fragment/CustomerDataTable.vue'
    import CustomerDetails from './fragment/CustomerDetails.vue'
    import CustomerForm from './fragment/CustomerForm.vue'
    import {
        Tabs,
        TabsContent,
        TabsList,
        TabsTrigger,
    } from '@/components/ui/tabs'
    import StickyFooterLayout from '@/layout/StickyFooterLayout.vue'

    const {
        subMenuTabs,
        customers,
        activeTab,
        activeCustomer,
        switchTab,
        createCustomer,
        updateCustomer,
        deleteCustomer,
        getSingleCustomer,
    } = useCustomer()
    const { deleting, showConfirmDialog, handleDelete, confirmDelete } =
        useConfirmDialog()
</script>

<template>
    <AppLayout activeRouteName="app.customer">
        <div class="p-8 overflow-y-auto">
            <CustomerDataTable
                :customers="customers"
                :activeCustomer="activeCustomer"
                @rowClick="(_customer) => getSingleCustomer(_customer.id)"
            />
        </div>

        <template #sidebar>
            <Tabs
                class="w-full"
                v-model="activeTab"
                @update:model-value="switchTab"
            >
                <StickyFooterLayout>
                    <template #header>
                        <TabsList class="w-full rounded-none">
                            <TabsTrigger
                                v-for="tab in subMenuTabs"
                                :key="tab.value"
                                class="data-[state=active]:shadow-none data-[state=active]:bg-transparent"
                                :value="tab.value"
                            >
                                <div
                                    class="flex items-center gap-2 transition-opacity duration-200 hover:opacity-80"
                                >
                                    <component :is="tab.icon" class="w-4 h-4" />
                                    {{ tab.label }}
                                </div>
                            </TabsTrigger>
                        </TabsList>
                    </template>

                    <TabsContent value="details" class="mt-0">
                        <CustomerDetails
                            :customer="activeCustomer"
                            :handleDelete="handleDelete"
                        />
                    </TabsContent>
                    <TabsContent value="create" class="mt-0">
                        <CustomerForm
                            :submitForm="createCustomer"
                            name="createForm"
                            ref="customerCreateFormRef"
                        />
                    </TabsContent>
                    <TabsContent value="edit" class="mt-0">
                        <CustomerForm
                            :submitForm="updateCustomer"
                            name="editForm"
                            ref="customerEditFormRef"
                        />
                    </TabsContent>
                </StickyFooterLayout>
            </Tabs>
        </template>
        <ConfirmDialog
            title="Are you sure you want to delete this customer?"
            description="This action will move the customer to the trash and can be restored within 30 days."
            v-model="showConfirmDialog"
            :deleting="deleting"
            @confirm="confirmDelete(deleteCustomer)"
        />
    </AppLayout>
</template>
