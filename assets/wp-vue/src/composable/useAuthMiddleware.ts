...
export const useAuthMiddleware = () => {
   ...
   */
  const checkRouteAccess = async (to: RouteLocationNormalized): Promise<boolean> => {
    ...
    const { routesByRoles } = useRouteVisibility({ userRoles: computed(() => userStore.authUser?.roles || []) })
    ...
    const isRouteAllowed = routesByRoles.value.some(route => {
      ...
      if (route.submenus) {
        return route.submenus.some(submenu => submenu.name === routeName)
      }
